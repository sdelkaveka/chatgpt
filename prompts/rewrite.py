...
async def get_rewrite_response(request: Request):
    ...
    dropdown4 = str(data.get('dropdown4', ''))    # Версия модели чата GPT
    if dropdown4.strip():
        model_chat_gpt = dropdown4                  # Версия модели чата GPT
    ...
    prompt = (
        ...
        'информацию, чтобы текст стал уникальным на 99%. Перепишите '
        'предложения, используя синонимы и другой лексический материал. '
        'Перераспределите фразы и слова в предложениях, измените порядок '
        'слов. Заменяйте ключевые слова и фразы на их синонимы или близкие '
        'по значению. При этом обязательно следите за тем, чтобы смысл '
        'оставался неизменным. Внесите дополнительные сведения. Перепишите '
        'абзацы в другом порядке или даже измените структуру текста, чтобы '
        'придать ему новый логический ход. Если текст содержит специфическую '
        'терминологию, попробуйте заменить ее на аналогичные понятия или '
        'термины. Вот исходный текст:'
    )
